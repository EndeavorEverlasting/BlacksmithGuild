...
- ForgeContinue, Auto Launch Nav, new-game, Steam-mediated, and future launch paths must use the same observer-first run context for Play/Continue, calibration, Safe Mode, Caution, Steam broker, other launcher windows, and game handoff; every correlated surface is recorded, each action target is identity-frozen, Steam and unknown windows remain observation-only, multitasking stays background-safe and mouse-independent, and fresh transition evidence is required before success.
