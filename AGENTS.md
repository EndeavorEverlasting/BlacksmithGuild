...
- Launcher Play/Continue selection must use a fresh unique process/window identity (exact PID/HWND preferred; unique process name or S1/S2 delta allowed), freeze it for the operation, remain background-safe and mouse-independent by default, and require explicit task authority plus post-action verification before any foreground, cursor, or coordinate fallback.
