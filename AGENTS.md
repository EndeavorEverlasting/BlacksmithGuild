...
- ForgeContinue, Auto Launch Nav, new-game, Steam-mediated, and future launch paths must use the same observer-first run context for Play/Continue, calibration, Safe Mode, Caution, Steam broker, other launcher windows, and game handoff; exact PID/HWND preferred; unique process name or S1/S2 delta allowed; every correlated surface is recorded, each action target is identity-frozen, Steam and unknown windows remain observation-only, multitasking stays background-safe and mouse-independent by default, and fresh transition evidence is required before success; the window observer may retire only after a same-run runtime-observer attachment is acknowledged, launcher handoff and MapTransition are not campaign readiness, campaign release requires campaignReady:true, canPollFileInbox:true, and a fresh 60-second stable map-ready interval, and the readiness cascade grants no gameplay authority.
