...
def main()->int:
    paths={
      ...
      'stale_progress_validator':ROOT/'scripts/tbg/Test-TbgStalePrRecoveryProgress.ps1','stale_progress_e2e':ROOT/'scripts/tbg/Test-TbgStalePrRecoveryProgressE2E.ps1',
    ...
    for key in ('endToEndProfiles','endToEndContract','endToEndEntrypoint','sprintCapsuleContract','consumerHandoffRegistry','stalePrRecoveryProgressValidator'): require(key in manifest['paths'],f'manifest missing {key}')
    ...
    journeys={j['id']:j for j in profiles['journeys']}
    require('stale-pr-recovery-progress' in journey_ids,'stale PR recovery progress journey missing')
    stale_journey=journeys['stale-pr-recovery-progress']
    require(stale_journey.get('kind')=='powershell','stale PR recovery progress must use the bounded PowerShell E2E interface')
    require(stale_journey.get('script')=='scripts/tbg/Test-TbgStalePrRecoveryProgressE2E.ps1','stale PR recovery progress must use the E2E adapter')
    require(stale_journey.get('contract')=='.tbg/workflows/stale-pr-recovery-automation.contract.json','stale PR recovery progress must bind the canonical recovery contract')
    ...
    default_profile=next(p for p in profiles['profiles'] if p['id']=='default-static')
    require('stale-pr-recovery-progress' in default_profile['journeyIds'],'default-static must gate stale PR recovery progress')
    ...
    for key in ('ps_test','ps_run','ps_capsule','stale_progress_e2e'):
