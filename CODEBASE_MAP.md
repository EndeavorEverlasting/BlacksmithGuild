...
## Live runtime proof admission

- `.tbg/workflows/live-runtime-proof-admission.contract.json` — machine-owned terminal-state and proof-level contract. `live_fresh_launch` rejects `skipLaunch=true` and requires the complete fresh correlated launcher-to-behavior chain.
- `.tbg/harness/live-runtime-proof-artifacts.registry.json` — artifact roles, naming, freshness, local retention, and required-for-pass evidence.
- `.tbg/harness/fixtures/live-runtime-proof-admission.fixtures.json` — positive and negative proof-admission cases including skip-launch, ACK-only, stale evidence, broken correlation, and complete live pass.
- `scripts/tbg/Test-TbgLiveRuntimeProofAdmission.ps1` — deterministic validator/proof reducer; ignores model-supplied proof promotion.
- `.tbg/harness/test-catalog.d/core/live-runtime-proof-admission.test.json` — one-click discovery entry.
- `.tbg/skills/runtime-evidence-certification/SKILL.md` — scoped procedure for using the admission gate before runtime claims.
- `docs/operator/live-runtime-proof-admission.md` — human-readable working/broken/missing boundaries and handoff contract.
- `.githooks/pre-push` — static proof-admission + harness-completeness + whitespace gate before push.
- `.github/workflows/live-runtime-proof-admission.yml` — PowerShell Core and Windows PowerShell 5.1 CI.
- `artifacts/latest/live-runtime-proof-admission/` — ignored latest result/report generated by the validator.

## Save compatibility classification

- `.tbg/state/save-compatibility.registry.json` — canonical save role/version policy, approved alias pair, exact-match gate, and post-create observation contract.
- `.tbg/workflows/save-compatibility-classification.contract.json` — read-only catalog/gate workflow and launcher/runtime consumer boundary.
- `.tbg/harness/save-compatibility-artifacts.registry.json` — save record, result, report, event, privacy, and retention roles.
- `.tbg/harness/fixtures/save-compatibility.fixtures.json` — exact/newer/older/unknown/ambiguous/disposable-name and approved-pair cases.
- `scripts/tbg/Invoke-TbgSaveCompatibility.ps1` — read-only save catalog/target gate; parses version evidence and SHA-256 without loading or mutating a save.
- `scripts/tbg/Test-TbgSaveCompatibility.ps1` — focused validator proving classification, pair integrity, role/version separation, and unchanged fixture bytes/write timestamps.
- `.tbg/harness/test-catalog.d/core/save-compatibility.test.json` — `ForgeTest.cmd` static discovery entry.
- `ForgeSaveCompatibility.cmd` — operator entry; defaults to read-only catalog mode and forwards explicit gate arguments.
- `docs/operator/save-compatibility.md` — human-readable classification and next-lane contract.
...
- `docs/evidence/runtime-context/` — bounded remote reconstruction packets; raw logs and crash dumps stay ignored/local.
...
- Live proof admission: `pwsh -NoProfile -File scripts/tbg/Test-TbgLiveRuntimeProofAdmission.ps1`.
