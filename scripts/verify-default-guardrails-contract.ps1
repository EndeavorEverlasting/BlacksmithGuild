...
Assert-TextContains -RelativePath $guardrails -Needle 'Focused route proof precondition' -Why 'route proof requires focused gameplay conditions'
Assert-TextContains -RelativePath $guardrails -Needle 'timePaused=true blocks route proof unless the harness owns unpause and focus recovery.' -Why 'paused campaign time must block route proof'
Assert-TextContains -RelativePath $guardrails -Needle 'keep Bannerlord focused during the route execution window' -Why 'focus lifecycle must be explicit'
Assert-TextContains -RelativePath $guardrails -Needle 'focus_lifecycle_unowned' -Why 'focus blocker classification must be explicit'
Assert-TextContains -RelativePath $guardrails -Needle 'Fast Continue launch selection verifier' -Why 'fast Continue regression must be tracked'
Assert-TextContains -RelativePath $guardrails -Needle 'Focused route proof lifecycle verifier' -Why 'route lifecycle verifier must be tracked'
...
Assert-TextContains -RelativePath $contamination -Needle 'Bannerlord loses foreground focus during route execution' -Why 'lost focus must contaminate route proof'
Assert-TextContains -RelativePath $contamination -Needle 'timePaused=true during route execution' -Why 'paused campaign time must contaminate route proof'
Assert-TextContains -RelativePath $contamination -Needle 'terminal wait substitutes for in-game execution ownership' -Why 'terminal wait mental model must be rejected'
Assert-TextContains -RelativePath $contamination -Needle 'focus_lifecycle_unowned' -Why 'focus lifecycle blocker must be documented'
Assert-TextContains -RelativePath $contamination -Needle 'autonomous route execution' -Why 'autonomous route claim must be forbidden when contaminated'
Assert-TextContains -RelativePath $contamination -Needle 'route brain may have selected a destination' -Why 'valid partial claim must be documented'
Assert-TextContains -RelativePath $contamination -Needle 'fast Continue launch selection' -Why 'fast Continue repair target must be documented'
