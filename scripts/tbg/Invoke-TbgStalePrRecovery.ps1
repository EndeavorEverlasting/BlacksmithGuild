...
foreach ($command in @($plan.localProof.commands)) {
    Add-TbgInstruction -List $instructions -Stage 'evidence' -Subject 'The coordinator' -Verb 'must run' -Object $command -Condition 'before the local floor can be marked verified' -Evidence 'the command output must identify the current repository state' -Command ([string]$command) -Sentence "The coordinator must run '$command' before the local floor can be marked verified, and the coordinator must retain the command output as repository-state evidence."
}
...
foreach ($entry in $selectedEntries) {
    ...
    $risk = ConvertTo-TbgWords $entry.risk
    ...
    Add-TbgInstruction -List $instructions -Stage 'bounded_plan' -Subject "The PR #$pr recovery agent" -Verb 'must classify' -Object 'each commit, path, hunk, and useful comment' -Condition "under the $classification classification and $risk risk" -Evidence "strategy: $strategy" -Sentence "The PR #$pr recovery agent must classify each commit, path, hunk, and useful comment as keep, superseded, reject, owner review, or runtime proof before the agent applies the $strategy strategy."

    $usefulShas = @($entry.usefulShas)
    $shaText = ConvertTo-TbgQuotedList $usefulShas
    $actionCommand = ''
    if ([string]$entry.strategy -eq 'selective_path_replay' -and $usefulShas.Count -gt 0) {
        $actionCommand = "git cherry-pick -x $($usefulShas[0])"
    }
...
$selectedDependencies = @($selectedWaves | ForEach-Object { @(Get-TbgValue -Object $_ -Name 'blockedBy' -Default @()) } | Select-Object -Unique)
$status = 'ready'
$verdict = 'READY'
...
if ($PrNumber -gt 0 -and $selectedEntries.Count -eq 0) {
...
elif (-not $LocalFloorVerified -and $Wave -ne '0') {
    $status = 'blocked'
    $verdict = 'BLOCKED'
    $terminalState = 'BLOCKED_local_floor_unverified'
    ...
    $firstPr = @($selectedEntries | Select-Object -First 1)
    if ($firstPr.Count -gt 0) {
        $nextCommand = "gh pr view $([int]$firstPr[0].pr) --json number,title,state,isDraft,mergeable,baseRefName,headRefName,headRefOid,commits,files,reviews,comments"
    }
    else {
...
$resultDraft = [pscustomobject][ordered]@{
    schema = 'TbgStalePrRecoveryInstructionResult.v1'
    profileId = [string]$contract.id
    action = 'InvokeStalePrRecovery'
    generatedUtc = [DateTime]::UtcNow.ToString('o')
    repository = [string]$plan.repository
    planPath = $PlanPath
    selectedWave = $Wave
    selectedPr = $PrNumber
    localFloorVerified = [bool]$LocalFloorVerified
    status = $status
    verdict = $verdict
    terminalState = $terminalState
    blockedReason = $blockedReason
    nextPatchHint = $nextCommand
    selectedTargets = @($selectedEntries | ForEach-Object { [int]$_.pr })
    safeParallelWaves = @($selectedWaves | ForEach-Object { @(Get-TbgValue -Object $_ -Name 'parallelWith' -Default @()) } | Select-Object -Unique)
    sourceFiles = @($PlanPath, $contractRelative)
...
$summarySentence = if ($verdict -eq 'READY') {
    "The stale PR recovery harness prepared $($instructions.Count) bounded instructions for $selectedLabel and selected '$nextCommand' as the next command."
}
else {
    "The stale PR recovery harness blocked $selectedLabel because $blockedReason The harness selected '$nextCommand' as the next command."
}

$result = [pscustomobject][ordered]@{
    schema = $resultDraft.schema
    profileId = $resultDraft.profileId
    action = $resultDraft.action
    generatedUtc = $resultDraft.generatedUtc
    repository = $resultDraft.repository
    planPath = $resultDraft.planPath
    selectedWave = $resultDraft.selectedWave
    selectedPr = $resultDraft.selectedPr
    localFloorVerified = $resultDraft.localFloorVerified
    status = $resultDraft.status
    verdict = $resultDraft.verdict
    terminalState = $resultDraft.terminalState
    blockedReason = $resultDraft.blockedReason
    nextCommand = $nextCommand
    selectedTargets = $resultDraft.selectedTargets
    safeParallelWaves = $resultDraft.safeParallelWaves
    effectivePolicy = $effectivePolicy
    englishSummary = "$policyEnglish $summarySentence"
    sourceFiles = $resultDraft.sourceFiles
    artifacts = $resultDraft.artifacts
    instructions = $resultDraft.instructions
}
...
$report.Add('# Stale PR recovery instruction report')
$report.Add('')
$report.Add($result.englishSummary)
$report.Add('')
$report.Add("The repository is '$($result.repository)'.")
$report.Add("The harness selected '$selectedLabel' and reached terminal state '$terminalState'.")
if (-not [string]::IsNullOrWhiteSpace($blockedReason)) { $report.Add("The harness is blocked because $blockedReason") }
$report.Add("The next command is '$nextCommand'.")
$report.Add('')
$report.Add('## Ordered instructions')
foreach ($instruction in $instructions) {
    $report.Add('')
    $report.Add("$($instruction.sequence). $($instruction.sentence)")
    if (-not [string]::IsNullOrWhiteSpace([string]$instruction.command)) {
        $report.Add('')
        $report.Add('```text')
        $report.Add([string]$instruction.command)
        $report.Add('```')
    }
}
$report.Add('')
...
$handoff.Add('# Stale PR recovery handoff')
$handoff.Add('')
$handoff.Add("The next agent must inspect '$PlanPath' and '$contractRelative'.")
$handoff.Add("The next agent must treat '$terminalState' as the current terminal state.")
$handoff.Add("The next agent must run '$nextCommand' and preserve its output.")
$handoff.Add('The next agent must not close a stale pull request, delete a branch or worktree, or reuse historical evidence as current proof.')
$handoff.Add("The next agent must inspect '$resultPath', '$reportPath', '$eventsPath', and '$progressPath'.")
