...
function GitText([string[]]$Args){$out=& $git -C $root @Args 2>&1;if($LASTEXITCODE){throw "git failed: $($out -join ' ')"};($out-join "`n").Trim()}
...
$asReady=(-not $dirty -and $Blockers.Count -eq 0);$sasReady=($ReadyForSysAdminSuite -and $asReady -and -not [string]::IsNullOrWhiteSpace($SysAdminSuiteOperation))
$capsule=[ordered]@{schema='tbg.sprint-capsule.v1';generatedUtc=[DateTime]::UtcNow.ToString('o');producer='EndeavorEverlasting/BlacksmithGuild';sprint=[ordered]@{id=$SprintId;title=$Title;lane=$Lane;mission=$Mission};git=[ordered]@{branch=$branch;headCommit=$head;dirty=$dirty};scope=[ordered]@{ownedPaths=@($OwnedPaths);forbiddenScope=@($ForbiddenScope)};validation=[ordered]@{commands=@($Validation);skipped=@($SkippedChecks)};proof=[ordered]@{level=$ProofLevel;ceiling=$ProofCeiling;claimsNotMade=@($ClaimsNotMade)};handoff=[ordered]@{completed=@($Completed);remaining=@($Remaining);blockers=@($Blockers);receivingConsumerMustReinspectState=$true};consumers=[ordered]@{agentSwitchboard=[ordered]@{ready=$asReady;authority='coordination-only';reason=$(if($asReady){'Clean committed state with no declared blockers.'}else{'Dirty state or blockers require review.'})};sysAdminSuite=[ordered]@{ready=$sasReady;authority='explicit-tandem-consumer-only';operation=$SysAdminSuiteOperation;reason=$(if($sasReady){'Explicit authorized tandem operation.'}else{'No explicit ready tandem operation.'})}};artifactRefs=@($ArtifactRefs);nextCommand=$NextCommand}
