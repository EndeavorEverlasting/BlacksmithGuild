...
$latestEventsPath = Join-Path $EvidenceRoot 'visible-trade-proof.events.jsonl'
...
try {
    ...
    $validators = @(
        ...
        'scripts\tbg\Test-TbgStateEnvelope.ps1',
    ...
    if (-not $diagnosticOnly -and -not $SkipLaunch) {
        ...
        $gatePath = Join-Path $PSScriptRoot 'tbg\Assert-TbgGameCompatibilityGate.ps1'
        if (-not (Test-Path -LiteralPath $gatePath)) {
            throw 'BLOCKED_GAME_BUILD_UNVALIDATED:compatibility_gate_missing'
        }
        $gateResult = & $gatePath `
            -Gate runtime-proof `
            -RepoRoot $RepoRoot `
            -BannerlordRoot $bannerlordRoot `
            -BuiltDllPath $localDllPath `
            -InstalledDllPath $installedDllPath `
            -NoExit `
            -PassThru
        if (-not $gateResult -or -not [bool]$gateResult.allowed) {
            $gateState = if ($gateResult) { [string]$gateResult.terminalState } else { 'BLOCKED_game_compatibility_result_missing' }
            throw "BLOCKED_GAME_BUILD_UNVALIDATED:$gateState"
        }
        Write-Event -Stage launch -Status info -Subject 'coordinator' -Action 'compatibility-gate' -Object 'runtime-proof' `
            -Evidence ([string]$gateResult.terminalState) `
            -Sentence "The game compatibility gate passed with state $($gateResult.terminalState)."

        $launchAttemptUtc = (Get-Date).ToUniversalTime()
        try {
            & powershell.exe -NoProfile -ExecutionPolicy Bypass `
                -File (Join-Path $PSScriptRoot 'invoke-forge-launch-operator.ps1') `
                -RepoRoot $RepoRoot -LaunchIntent continue -TimeoutSec $AttachTimeoutSec 2>&1 | Out-Null
        } catch {
    ...
    # ═══════════════════════════════════════════════════════════════
    # STAGE: closeout (write result/handoff before capsule consumes them)
    # ═══════════════════════════════════════════════════════════════
    ...
    function Write-TbgVisibleTradeCloseoutArtifacts {
        param(
            $Publication,
            $Capsule
        )
        ...
        $proofObj = [ordered]@{
            ...
            checks = [ordered]@{
                ...
                capsuleGenerated = $null -ne $Capsule
                remotePublished = $null -ne $Publication -and $Publication.published
            }
        }

        Write-AtomicUtf8Json -Value $result -Path $resultPath
        Write-AtomicUtf8Json -Value $proofObj -Path $proofPath
        if ($Capsule) { Write-AtomicUtf8Json -Value $Capsule -Path $capsuleJsonPath }
...
"@
    ...
    try {
        $script:result = Write-TbgVisibleTradeCloseoutArtifacts -Publication $null -Capsule $null
    } catch {
    ...
    # ═══════════════════════════════════════════════════════════════
    if ($diagnosticOnly) {
        Write-Event -Stage remote-publish -Status skipped -Subject 'coordinator' -Action 'publish-evidence' -Object 'remote' `
            -Sentence 'Remote publication skipped in diagnostic/dry-run mode.'
    } elseif ($capsuleResult -and $capsuleResult.capsulePath) {
        ...
        try {
            ...
            if ($publicationResult.published) {
                ...
                if ($terminalState -eq 'PASS_VISIBLE_TRADE_PROVEN') {
    ...
    try {
        $script:result = Write-TbgVisibleTradeCloseoutArtifacts -Publication $publicationResult -Capsule $capsuleResult
        Copy-Item -LiteralPath $progressPath -Destination $latestProgressPath -Force -ErrorAction SilentlyContinue
        Copy-Item -LiteralPath $eventsPath -Destination $latestEventsPath -Force -ErrorAction SilentlyContinue
        ...
        Write-Host "Final closeout write error: $($_.Exception.Message)" -ForegroundColor Yellow
    }
