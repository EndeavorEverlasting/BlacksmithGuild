...
.DESCRIPTION
  ...
  Copied from SysAdminSuite (tools/Add-Utf8Bom.ps1) — same contract.
...
.EXAMPLE
  .\scripts\tools\Add-Utf8Bom.ps1
  .\scripts\tools\Add-Utf8Bom.ps1 -Fix
  .\scripts\tools\Add-Utf8Bom.ps1 -Path .\scripts -Fix
#>
...
if (-not $Path) {
    if ($PSScriptRoot) { $Path = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent }
    else { $Path = $PWD.Path }
}

$exclude = @('.git', 'node_modules', '__pycache__', 'Output', 'Archive', 'dist', 'bin', 'obj')
...
Write-Host ''
Write-Host '=== Add-Utf8Bom ===' -ForegroundColor Cyan
Write-Host "Root : $Path"
Write-Host "Mode : $(if ($Fix) { 'FIX' } else { 'DRY-RUN (use -Fix to apply)' })"
Write-Host ''
