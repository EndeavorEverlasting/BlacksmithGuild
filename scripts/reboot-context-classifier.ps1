...
function New-RebootNormalizedContext {
    ...
    $ctx = [ordered]@{
        ...
        travelClockRunning = (ConvertTo-RebootBool $execution.travelClockRunning)
        movementIntentSet = (ConvertTo-RebootBool $execution.movementIntentSet)
        partyMovedDistanceBucket = $movementBucket
        partyMovementObserved = [bool]((ConvertTo-RebootBool $summary.visibleMechanicsProven) -or ($checkpoints -contains 'party_movement_observed') -or ($movementBucket -ne 'zero'))
