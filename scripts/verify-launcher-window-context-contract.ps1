...
foreach ($sourceCheck in @(
    ...
    @{ file = $frozenNav; text = 'launcher_hosted_loading_handoff' },
    @{ file = $frozenNav; text = '$clickVerifyBudgetMs = 30000' },
    @{ file = $frozenNav; text = '$maxAttempts = if ($AllowFocusSteal -or -not $RespectUserForeground) { 1 } else { 2 }' },
