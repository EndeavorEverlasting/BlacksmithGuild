...
$registry = 'src\BlacksmithGuild\DevTools\DevCommandRegistry.cs'
$bus = 'src\BlacksmithGuild\DevTools\DevCommandBus.cs'
$commandNames = 'scripts\dev-command-names.ps1'
...
Assert-TextMatches -RelativePath $authority -Pattern '(?s)SetMapTradeAutomationCommand.*?SetEngineMode\(\s*EngineToggleKey\.MapTrade,\s*EngineToggleMode\.Automation' -Label 'targeted MapTrade Automation command' -Why 'unattended validation must not enable every engine'
Assert-TextContains -RelativePath $registry -Needle 'EngineToggleAuthority.SetMapTradeAutomationCommand' -Why 'targeted authority command must be inbox registered'
Assert-TextContains -RelativePath $bus -Needle 'case EngineToggleAuthority.SetMapTradeAutomationCommand:' -Why 'targeted authority command must be dispatched'
Assert-TextContains -RelativePath $commandNames -Needle "'SetMapTradeAutomation'" -Why 'external runners must recognize the command'
...
Assert-TextContains -RelativePath $doc -Needle 'SetMapTradeAutomation' -Why 'agent-runnable targeted control must be documented'
