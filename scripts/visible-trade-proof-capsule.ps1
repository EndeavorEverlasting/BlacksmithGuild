...
function New-TbgVisibleTradeProofCapsule {
    ...
    if (Test-Path -LiteralPath $EventsJsonl) {
        $rawEvents = Get-Content -LiteralPath $EventsJsonl -Raw -ErrorAction SilentlyContinue
        $lines = @()
        if (-not [string]::IsNullOrWhiteSpace($rawEvents)) {
            $lines = @($rawEvents -Split "`n" | Where-Object { $_.Trim().Length -gt 0 })
        }
        foreach ($line in $lines) {
            try {
                ...
                $eventsData += @{ raw = (Sanitize-TbgCapsuleString -Text $line -RepoRoot $repoRoot) }
            }
        }
    }
...
function Sanitize-TbgCapsuleObject {
    ...
    $sanitized = Sanitize-TbgCapsuleString -Text $json -RepoRoot $RepoRoot
    try {
...
function Sanitize-TbgCapsuleMarkdown {
    ...
    return Sanitize-TbgCapsuleString -Text $content -RepoRoot $RepoRoot
}
